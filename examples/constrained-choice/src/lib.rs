...
use inferlet::{Context, greedy, model};
...
impl inferlet::Guest for App {
    fn run(args: Vec<String>) -> Result<String, String> {
        ...
        let choices: Vec<Vec<u32>> = list.split('|').map(|c| model::tokenize(&format!(" {c}"))).collect();
        ...
        let odds: Vec<String> = d
            ...
            .iter()
            .zip(&d.probs)
            .map(|(t, p)| format!("{:?} {:.2}", model::detokenize(&[*t]), p))
            .collect();
        Ok(format!(
            "{:?}\n    first token: {}",
            model::detokenize(&out),
            odds.join(", ")
        ))
    }
}
